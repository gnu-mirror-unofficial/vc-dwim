...
local-checks-available = \
  po-check copyright-check m4-check \
...
useless_if = $(srcdir)/build-aux/useless-if-before-free
sc_avoid_if_before_free:
	@if test -f $(useless_if); then					\
	  $(useless_if) $(useless_free_options)				\
	    $$($(VC_LIST_EXCEPT)) &&					\
...
sc_tight_scope:
	@grep $@ $(srcdir)/src/Makefile.am > /dev/null 2>&1		\
	  && $(MAKE) -C src $@ || :
